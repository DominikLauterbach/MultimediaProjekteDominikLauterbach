...

        {
            ...
            while (m_window.pollEvent(event))
            {
                if (event.type == sf::Event::Resized)
                {
                    ...
                    tgui::FloatRect rect(view.getCenter().x - view.getSize().x / 2.f,
                                         view.getCenter().y - view.getSize().y / 2.f,
                                         view.getSize().x,
                                         view.getSize().y);
                    m_gui.setAbsoluteView(rect);
                }
...
bool Game::init()
{
    ...
    m_gameStateManager.registerState("BindingState", make_shared<BindingState>(&m_gameStateManager, this));
    ...
    m_gameStateManager.registerState("LoseState", make_shared<LoseState>(&m_gameStateManager, this));
